...
def _draw_schematic_regime(ax: plt.Axes, topology: str) -> None:
    """Circuit schematic for one of the four interaction modes.

    Visual style mirrors _draw_schematic_hub_lateral from
    pcca_hublateral_simulations.py: rounded-rectangle nodes, coloured arrows,
    and labelled source nodes.  Self-contained so no cross-file import is needed.

    topology  'hub_z'       Mode 1 b-topology, pCCA(A,B|Z)
              'hub_b'       Mode 1 b-topology, pCCA(A,Z|B)
              'collider'    Mode 2  X→Z←Y
              'mediator'    Mode 3  relay+bypass, same g_X in X
              'independent' Mode 4  exogenous Z
    """
    ax.set_xlim(0, 1); ax.set_ylim(0, 1); ax.axis('off')
    _bs = dict(boxstyle='round,pad=0.15', lw=1.0, zorder=3)

    def _node(xy, lbl, fc='#c6dbef', ec='#084594', fs=9.5):
        ax.text(xy[0], xy[1], lbl, ha='center', va='center',
                fontsize=fs, fontweight='bold', zorder=4,
                bbox=dict(fc=fc, ec=ec, **_bs))

    def _arr(p0, p1, col='#333333', lw=1.5, ls='solid', d=0.07):
        p0, p1 = np.asarray(p0, float), np.asarray(p1, float)
        dv = p1 - p0; nm = np.linalg.norm(dv)
        if nm < 1e-9: return
        u = dv / nm
        ax.annotate('', xy=tuple(p1 - d * u), xytext=tuple(p0 + d * u),
                    arrowprops=dict(arrowstyle='->', color=col, lw=lw,
                                   linestyle=ls, mutation_scale=10), zorder=2)

    # ── Mode 1 — hub-lateral (b-topology) ────────────────────────────────────
    if topology in ('hub_z', 'hub_b'):
        # Replicates _draw_schematic_hub_lateral for the b-branch config:
        #   s_ZG → {Z, A1, B}   (hub; does NOT reach A2)
        #   s_A2B → {A2, B}     (lateral)
        P = {'Z':  (0.12, 0.50), 'A1': (0.50, 0.80),
             'A2': (0.50, 0.20), 'B':  (0.90, 0.50)}
        hub_pos = (0.30, 0.50)
        lat_pos = (0.68, 0.27)

        for key, lbl, fc, ec in [
            ('Z',  'Z',  '#fdd0a2', '#8c2d04'),
            ('A1', 'A₁', '#c6dbef', '#084594'),
            ('A2', 'A₂', '#c6dbef', '#084594'),
            ('B',  'B',  '#fcbba1', '#a50f15'),
        ]:
            _node(P[key], lbl, fc=fc, ec=ec)

        _node(hub_pos, 's_ZG',  fc='#d9f0d3', ec=C_HUB,  fs=8.5)
        _node(lat_pos, 's_A2B', fc='#fdeacb', ec=C_COMM, fs=8.5)

        for tgt in ('Z', 'A1', 'B'):      # hub reaches Z, A1, B (NOT A2)
            _arr(hub_pos, P[tgt], col=C_HUB,  lw=1.4)
        for tgt in ('A2', 'B'):           # lateral reaches A2, B
            _arr(lat_pos, P[tgt], col=C_COMM, lw=1.4)

        cond = 'pCCA(A,B|Z) → lateral recovered' if topology == 'hub_z' \
               else 'pCCA(A,Z|B) → hub recovered'
        ax.set_title(f'Mode 1 — Hub-lateral  (b-topology)\n{cond}',
                     fontsize=8.5, fontweight='bold', pad=3)

    # ── Mode 2 — collider (X → Z ← Y) ────────────────────────────────────────
    elif topology == 'collider':
        Px  = (0.15, 0.46); Py  = (0.85, 0.46); Pz  = (0.50, 0.46)
        Psx = (0.15, 0.84); Psy = (0.85, 0.84)

        _node(Psx, 's_X', fc='#c6dbef', ec='#084594', fs=8.5)
        _node(Psy, 's_Y', fc='#fcbba1', ec='#a50f15', fs=8.5)
        _node(Px,  'X\n(target)',   fc='#c6dbef', ec='#084594')
        _node(Py,  'Y\n(target)',   fc='#fcbba1', ec='#a50f15')
        _node(Pz,  'Z\n(collider)', fc='#fdd0a2', ec='#8c2d04')

        _arr(Psx, Px, col='#084594', lw=1.4)   # s_X → X
        _arr(Psx, Pz, col='#084594', lw=1.4)   # s_X → Z
        _arr(Psy, Py, col='#a50f15', lw=1.4)   # s_Y → Y
        _arr(Psy, Pz, col='#a50f15', lw=1.4)   # s_Y → Z

        ax.text(0.50, 0.24, 'X  ✗  Y\n(no direct coupling)',
                ha='center', va='center', fontsize=8.5,
                color='#888888', style='italic')

        ax.set_title('Mode 2 — Collider  (X→Z←Y)\n'
                     'pCCA > CCA  (spurious residual inflation)',
                     fontsize=8.5, fontweight='bold', pad=3)

    # ── Mode 3 — partial mediator (relay + bypass) ────────────────────────────
    elif topology == 'mediator':
        Px      = (0.15, 0.46); Py      = (0.85, 0.46); Pz = (0.50, 0.75)
        Prelay  = (0.50, 0.93)
        Pbypass = (0.50, 0.12)

        _node(Prelay,  's_relay',  fc='#d9f0d3', ec=C_HUB,  fs=8.5)
        _node(Pbypass, 's_bypass', fc='#fdeacb', ec=C_COMM, fs=8.5)
        _node(Px, 'X\n(single pool\nκ ≈ 1)', fc='#c6dbef', ec='#084594', fs=8)
        _node(Py, 'Y\n(target)',              fc='#fcbba1', ec='#a50f15')
        _node(Pz, 'Z\n(relay)',               fc='#fdd0a2', ec='#8c2d04')

        for tgt in (Px, Pz, Py):             # relay → X, Z, Y
            _arr(Prelay, tgt, col=C_HUB,  lw=1.4)
        for tgt in (Px, Py):                 # bypass → X, Y  (not Z)
            _arr(Pbypass, tgt, col=C_COMM, lw=1.4)

        # Same-g_X annotation inside X node region
        ax.text(0.50, 0.46,
                '(same g_X)', ha='center', va='center',
                fontsize=7, color='#444', style='italic')

        ax.set_title('Mode 3 — Partial mediator  (relay + bypass)\n'
                     'pCCA < CCA  (relay over-removed);  κ ≈ 1',
                     fontsize=8.5, fontweight='bold', pad=3)

    # ── Mode 4 — independent Z (exogenous confound) ───────────────────────────
    elif topology == 'independent':
        Px   = (0.18, 0.46); Py   = (0.55, 0.46); Pz   = (0.88, 0.46)
        PsXY = (0.37, 0.84); PsZ  = (0.88, 0.84)

        _node(PsXY, 's_XY', fc='#d9f0d3', ec=C_HUB,  fs=8.5)
        _node(PsZ,  's_Z',  fc='#e6d6ec', ec=C_COMP, fs=8.5)
        _node(Px,   'X\n(target)',  fc='#c6dbef', ec='#084594')
        _node(Py,   'Y\n(target)',  fc='#fcbba1', ec='#a50f15')
        _node(Pz,   'Z\n(⊥ X, Y)', fc='#fdd0a2', ec='#8c2d04')

        _arr(PsXY, Px, col=C_HUB,  lw=1.4)   # s_XY → X
        _arr(PsXY, Py, col=C_HUB,  lw=1.4)   # s_XY → Y
        _arr(PsZ,  Pz, col=C_COMP, lw=1.4)   # s_Z  → Z  (isolated)

        # Dotted independence marker between Y and Z
        ax.plot([Py[0] + 0.08, Pz[0] - 0.08], [0.46, 0.46],
                ls=':', color='#aaaaaa', lw=1.2)
        ax.text((Py[0] + Pz[0]) / 2, 0.53,
                '⊥', ha='center', va='center', fontsize=13, color='#aaaaaa')

        ax.set_title('Mode 4 — Independent Z  (exogenous)\n'
                     'pCCA ≈ CCA  (θ ≈ 0°,  r²_nuis ≈ 0)',
                     fontsize=8.5, fontweight='bold', pad=3)

    else:
        ax.text(0.5, 0.5, f'schematic for {topology!r} not implemented',
...
def fig_simulation_psth_latents(
    ...
    topology:        str   = 'hub_z',   # 'Z' => pCCA(A,B|Z);  'B' => pCCA(A,Z|B)
    save:            bool  = True,
) -> None:
    ...
    """
    _VALID = ('hub_z', 'hub_b', 'collider', 'mediator', 'independent')
    if topology not in _VALID:
        raise ValueError(f"topology must be one of {_VALID}, got {topology!r}")

    print(f"[Fig A  topology={topology}]  "
    ...
    if topology in ('hub_z', 'hub_b'):
        ...
        X_A, X_B, X_Z, time_vec, gt = gen_population_collider_trials(
            n_pool, n_pool, n_pool,
            n_trials=n_trials, T=T, t_onset=t_onset, sigma_priv=sigma_priv,
        )
    elif topology == 'mediator':
        X_A, X_B, X_Z, time_vec, gt = gen_population_mediator_trials(
            n_pool, n_pool, n_pool,
            n_trials=n_trials, T=T, t_onset=t_onset, sigma_priv=sigma_priv,
        )
    else:  # 'independent'
        X_A, X_B, X_Z, time_vec, gt = gen_population_independent_trials(
            n_pool, n_pool, n_pool,
            n_trials=n_trials, T=T, t_onset=t_onset, sigma_priv=sigma_priv,
        )

    # ── Route target pair and nuisance region ─────────────────────────────────
    if topology == 'hub_z':
        ...
        gt_p_prof = gt['rate_A2B']  # pCCA tracks lateral
        gt_p2_prof = gt['rate_A2B']
        gt_p_lbl = 'GT: lateral rate'
        gt_c_prof = gt['rate_ZG'] + gt['rate_A2B']  # CCA: hub+lat mixture
        gt_c_lbl = 'GT: hub+lat.'
        ...
        gt_p_prof = gt['rate_ZG']  # pCCA tracks hub
        gt_p2_prof = gt['rate_ZG']
        gt_p_lbl = 'GT: hub rate'
        gt_c_prof = gt['rate_ZG']
        gt_c_lbl = 'GT: hub rate'

    elif topology == 'collider':
        # X and Y are INDEPENDENT; Z is their collider
        X_tgt1, X_tgt2, X_nuis = X_A, X_B, X_Z
        tgt1_name = 'Region X  (private source s_X)'
        tgt2_name = 'Region Y  (private source s_Y)'
        nuis_name = 'Region Z  (collider)'
        sort_nuis = np.argsort(-(gt['g_ZX'] + gt['g_ZY']))
        beta_lbl1, beta_lbl2 = 'Z→X', 'Z→Y'
        # CCA baseline: X tracks its own source s_X (pre-onset)
        # pCCA: spurious direction from collider conditioning (no clean GT)
        gt_p_prof = None  # no interpretable GT for spurious pCCA
        gt_p2_prof = None
        gt_p_lbl = None
        gt_c_prof = gt['rate_X']  # CCA: X-side tracks s_X
        gt_c_lbl = 'GT: s_X rate'

    elif topology == 'mediator':
        # X has a single g_X direction for both relay and bypass
        X_tgt1, X_tgt2, X_nuis = X_A, X_B, X_Z
        tgt1_name = 'Region X  (single pool; κ ≈ 1)'
        tgt2_name = 'Region Y  (relay + bypass targets)'
        nuis_name = 'Region Z  (relay; captures s_relay)'
        sort_nuis = np.argsort(-gt['g_Z'])
        beta_lbl1, beta_lbl2 = 'Z→X', 'Z→Y'
        # pCCA removes relay → residual is bypass component
        gt_p_prof = gt['rate_bypass']  # pCCA: bypass rate
        gt_p2_prof = gt['rate_bypass']
        gt_p_lbl = 'GT: bypass rate'
        gt_c_prof = gt['rate_relay'] + gt['rate_bypass']  # CCA: full mixture
        gt_c_lbl = 'GT: relay+bypass'

    else:  # 'independent'
        # Z is entirely unrelated to X-Y coupling
        X_tgt1, X_tgt2, X_nuis = X_A, X_B, X_Z
        tgt1_name = 'Region X  (genuine coupling via s_XY)'
        tgt2_name = 'Region Y  (genuine coupling via s_XY)'
        nuis_name = 'Region Z  (independent; s_Z only)'
        sort_nuis = np.argsort(-gt['g_Z'])
        beta_lbl1, beta_lbl2 = 'Z→X', 'Z→Y'
        # pCCA ≈ CCA; both track s_XY
        gt_p_prof = gt['rate_XY']
        gt_p2_prof = gt['rate_XY']
        gt_p_lbl = 'GT: s_XY rate'
        gt_c_prof = gt['rate_XY']
        gt_c_lbl = 'GT: s_XY rate'
    # ── pCCA + CCA ────────────────────────────────────────────────────────────
    ...
    if topology in ('hub_z', 'hub_b'):
        # Ground-truth A1/A2 pool ordering with boundary annotation
        ...
        # Non-hub topologies: descend by loading magnitude (no pool boundary)
        bound1 = None
        sort_tgt1 = np.argsort(-gt['g_X'])
        if topology == 'collider':
            sort_tgt2 = np.argsort(-gt['g_Y'])
        elif topology == 'mediator':
            sort_tgt2 = np.argsort(-(np.abs(gt['g_Y_relay']) + np.abs(gt['g_Y_bypass'])))
        else:  # 'independent'
            sort_tgt2 = np.argsort(-gt['g_Y'])
        X_tgt1_z = _zscore_sim(X_tgt1)
        X_tgt2_z = _zscore_sim(X_tgt2)
    ...
    _TOPO_DESCS = {
        'hub_z':       'Mode 1 (b-topology) | pCCA(A,B|Z) → lateral; pCCA ↓ CCA',
        'hub_b':       'Mode 1 (b-topology) | pCCA(A,Z|B) → hub; pCCA ↓ CCA',
        'collider':    'Mode 2 (collider X→Z←Y) | pCCA ↑ CCA (spurious inflation)',
        'mediator':    'Mode 3 (mediator relay+bypass) | pCCA ↓ CCA (over-removal)',
        'independent': 'Mode 4 (independent Z) | pCCA ≈ CCA (θ ≈ 0°)',
    }
    noise_str = ('noise-free Poisson' if sigma_priv == 0.0
                 else f'sigma_priv = {sigma_priv:.1f}')
    gini_str  = (f'Gini: A1={gt["gini_A1"]:.2f}, A2={gt["gini_A2"]:.2f}'
                 if topology in ('hub_z', 'hub_b') else
                 f'Gini: X={gt["gini_X"]:.2f}')
    fig.suptitle(
        f'[Simulation — {_TOPO_DESCS[topology]},  {noise_str}]\n'
        f'pCCA ρ₁ = {res["rho_p"]:.4f}   |   CCA ρ₁ = {res["rho_c"]:.4f}   |   '
        f'n = {n_pool}/pool,  {n_trials} trials,  T = {T} bins '
        f'({int(DT_SEC * 1e3)} ms/bin)   |   {gini_str}',
        fontsize=10.5, fontweight='bold', y=0.97,
    )
    if save:
        os.makedirs(OUT_DIR, exist_ok=True)
        tag = f'npool{n_pool}_ntrials{n_trials}_spriv{sigma_priv:.1f}_{topology}'
...
def fig_regime_comparison(
    ...
    """Four-column × four-row connectivity-regime comparison figure.

    Mirrors fig_hublateral_cca_vs_pcca from pcca_hublateral_simulations.py
    in layout and visual style, but compares interaction MODES instead of
    parameter settings.

    ┌─────┬─────────────────────────────────────────────────────────────────┐
    │ Row │ Content                                                          │
    ├─────┼─────────────────────────────────────────────────────────────────┤
    │  0  │ Circuit schematic  (_draw_schematic_regime)                      │
    │  1  │ CCA compass: w_AB_cca (blue) and w_AZ_cca (red-orange) on the  │
    │     │ mode-appropriate reference plane; angle = theta_CCA printed      │
    │  2  │ pCCA compass: w_AB_pcca and w_AZ_pcca on the same plane;        │
    │     │ kappa = |cos(w_AB_pcca, w_AZ_pcca)| printed in title            │
    │  3  │ rho bar chart (PRIMARY): CCA vs pCCA ρ₁ with directional arrow  │
    └─────┴─────────────────────────────────────────────────────────────────┘

    The four columns are Mode 1 (hub_z), Mode 2 (collider),
    Mode 3 (mediator), Mode 4 (independent).

    Reference plane per mode
    ------------------------
    Mode 1: ground-truth plane span{u_hub_A, u_lat_A}.
    Modes 2–4: data-driven plane span{w_AB_cca, w_AZ_cca} — the two
    unconditional CCA coupling directions in X, which serve as the natural
    Cartesian frame when no ground-truth is available.
    """
    ...
    ncols = len(_REGIMES)
    fig   = plt.figure(figsize=(5.5 * ncols, 17.5))
    gs    = fig.add_gridspec(4, ncols, hspace=0.52, wspace=0.30,
                              height_ratios=[1.0, 1.65, 1.65, 1.0])
    ...
    for col, reg in enumerate(_REGIMES):
        ...
        # ── CCA and pCCA (X↔Z) for compass and kappa ─────────────────────────
        ...
        raw_dir = raw_cross_dir(A_flat, B_flat)

        # ── Reference plane ───────────────────────────────────────────────────
        if reg['topology'] == 'hub_z':
            E, phi = orthonormal_plane(gt['u_hub_A'], gt['u_lat_A'])
        else:
            E, phi = orthonormal_plane(w_AB_c, w_AZ_c)   # data-driven

        # ── Row 0: Circuit schematic ──────────────────────────────────────────
        ax0 = fig.add_subplot(gs[0, col])
        _draw_schematic_regime(ax0, reg['topology'])
        # Overwrite the title set inside _draw_schematic_regime with the
        # same content but also add the rho numbers (computed above)
        ax0.set_title(
            ax0.get_title() + f'\nρ_CCA = {rho_c[0]:.3f}   ρ_pCCA = {rho_p[0]:.3f}',
        ...
        _compass_population(
            ...
            angle=theta_c,
            title=f'{reg["label"]}\nCCA   θ = {theta_c:.1f}°',
            basis_labels=(reg['cats'][0], reg['cats'][1]),
        )
        ...
        _compass_population(
            ax2, w_AB_p, w_AZ_p, E,
            angle=theta_p,
            title=f'pCCA   θ = {theta_p:.1f}°   κ = {kappa:.2f}',
            basis_labels=(reg['cats'][0], reg['cats'][1]),
        )

        # ── Row 3: rho comparison (PRIMARY DIAGNOSTIC) ────────────────────────
        ax3 = fig.add_subplot(gs[3, col])
        bw  = 0.28
        rc  = rho_cca_all[-1]
        rp  = rho_pcca_all[-1]

        ax3.bar([0.0], [rc], bw, color=C_CCA, alpha=0.82,
                label=f'CCA   ρ={rc:.3f}')
        ax3.bar([0.38], [rp], bw, color=C_AZ, alpha=0.88,
                label=f'pCCA  ρ={rp:.3f}')

        if rp > rc + 0.02:
            diag, dcol = '↑  pCCA > CCA', '#e74c3c'
        elif rp < rc - 0.02:
            diag, dcol = '↓  pCCA < CCA', '#27ae60'
        else:
            diag, dcol = '≈  pCCA ≈ CCA', '#7f8c8d'

        ax3.text(0.19, max(rc, rp) + 0.05, diag,
                 ha='center', va='bottom', fontsize=9,
                 color=dcol, fontweight='bold')
        ax3.axhline(0, color='k', lw=0.6)
        ax3.set_xticks([0.0, 0.38])
        ax3.set_xticklabels(['CCA', 'pCCA'], fontsize=9)
        ax3.set_ylabel('ρ₁', fontsize=9)
        ax3.set_title('rho comparison\n(primary diagnostic)', fontsize=8.5)
        ax3.set_ylim(-0.08, max(rc, rp, 0.05) + 0.25)
        ax3.legend(fontsize=7, frameon=False, loc='upper right')
        for sp in ('top', 'right'): ax3.spines[sp].set_visible(False)

        # ── Optional weight-bar comparison (same API as fig_population_sweep) ─
        # Uncomment below if you want a 5th row showing projections
        # ax4 = fig.add_subplot(gs[4, col])
        # _wt_bars_population(ax4, w_AB_c, w_AZ_c, w_AB_p, w_AZ_p, E,
        #                      show_legend=(col == 0), cats=reg['cats'])

    # ── Row labels (left margin) ──────────────────────────────────────────────
    for y_frac, lbl in zip([0.880, 0.640, 0.395, 0.095],
                            ['Circuit\nschematic', 'CCA\ncompass',
                             'pCCA\ncompass', 'ρ\ncompare']):
    ...
    fig.suptitle(
        ...
        f'Expected: Mode 1 pCCA↓CCA (hub removed)  │  '
        f'Mode 2 pCCA↑CCA (collider)  │  '
        f'Mode 3 pCCA↓CCA (over-removed)  │  '
        f'Mode 4 pCCA≈CCA (irrelevant Z)',
...
def main() -> None:
    ...
    # NEW calls:
    for topo in ('hub_z', 'hub_b', 'collider', 'mediator', 'independent'):
        fig_simulation_psth_latents(
            n_pool=20, sigma_priv=0.0,
            n_trials=N_TRIALS, T=T_BINS, t_onset=T_ONSET,
            show_gt_overlay=True, topology=topo,
        )

    fig_regime_comparison(n_pool=20, sigma_priv=2.0)
    fig_sim_supplementary_comparison(n_pool=20, sigma_priv=2.0)
