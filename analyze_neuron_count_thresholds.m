...
results_dir = fullfile(base_dir, 'sessions_analysis_results');
...
for i = 1:n_sessions
    ...
    try
        ...
        for r = 1:length(regions)
            ...
            % CRITICAL: spike_data format is [neuron × time × trial]
            % We need the NEURON dimension (first dimension)
            if isfield(region_data, 'spike_data')
                spike_data = region_data.spike_data;
                n_neurons = size(spike_data, 1);  % First dimension = neurons
