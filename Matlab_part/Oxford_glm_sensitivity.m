...
% patterns suggest distributed coding across the population.
    ...
    glm_results_dir = fullfile(session_results_dir, sprintf('GLM_Analysis_Component_%d', component_to_analyze));
    cca_results_dir = session_results_dir;
    psth_results_dir = session_results_dir;
    ...
    region_pairs = {
        ...
        'MOs_STR', 'OLF_STR', 'STR_STRv', 'STR_LP','STR_VALVM','STR_VPMPO','STR_ILM','STR_HY'
    };
    % region_pairs = {
    %     'STR_STRv', 'STR_LP','STR_VALVM','STR_VPMPO','STR_ILM','STR_HY'
    % };
    ...
    for pair_idx = 1:length(region_pairs)
        ...
        for session_idx = 1:length(pair_data.sessions)
            ...
            try
                ...
                % Load CCA results
                cca_file = fullfile(cca_results_dir, sprintf('%s_analysis_results.mat', session_name));
                ...
                name1 = pair_data.region1;
                name2 = pair_data.region2;
                ...
                if isempty(pair_result_idx)
                    pair_name = sprintf('%s_%s', pair_data.region2, pair_data.region1);
                    name1 = pair_data.region2;
                    name2 = pair_data.region1;
                    % Extract GLM coefficients
                    beta1 = session_data.region2_coefficients;
                    beta2 = session_data.region1_coefficients;
                    pair_result_idx = find(strcmp(cca_results.region_pairs, pair_name), 1);
                    if isempty(pair_result_idx)
                ...
                % Load PSTH data
                % psth_file = fullfile(psth_results_dir, sprintf('%s_analysis_results.mat', session_name));
                % psth_data_struct = load(psth_file);
                psth_data = cca_data.region_data;
                
                % Extract neural data for selected neurons
                region1_trial_data = psth_data.regions.(name1).spike_data;
                region2_trial_data = psth_data.regions.(name2).spike_data;
                
                selected_neurons1 = psth_data.regions.(name1).selected_neurons;
                selected_neurons2 = psth_data.regions.(name2).selected_neurons;
                ...
                if ~strcmp(name1, pair_data.region1)
                    % Store session results
                    session_result = struct();
                    session_result.session_name = session_name;
                    session_result.region1_toprank = var_explained2_toprank;
                    % session_result.region1_random = var_explained1_random;
                    session_result.region2_toprank = var_explained1_toprank;
                    % session_result.region2_random = var_explained2_random;
                    session_result.original_r2_region1 = session_data.region2_stats.r_squared;
                    session_result.original_r2_region2 = session_data.region1_stats.r_squared;
...
function create_sensitivity_visualization(sensitivity_results, output_dir)
    ...
    if contains(sensitivity_results.region1, 'STR')
    ...
    elseif contains(sensitivity_results.region2, 'STR')
