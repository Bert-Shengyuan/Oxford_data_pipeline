...
%
    ...
    config.results_dir = fullfile(config.oxford_base_dir, 'sessions_cued_hit_long_results');
    config.output_dir = fullfile(config.oxford_base_dir, 'neuron_stability_analysis_cued');
    
    % Analysis Parameters - Theoretical Foundation
    config.reference_neurons = 100;          % Reference population size for "gold standard"
    ...
    config.neuron_counts = [5, 10, 15, 20,25, 30, 35,40,45,50, 60, 70, 80, 90]; % Test populations
    % 
...
% population sizes and data availability
    ...
    results_pattern = fullfile(config.results_dir, '*_analysis_results.mat');
