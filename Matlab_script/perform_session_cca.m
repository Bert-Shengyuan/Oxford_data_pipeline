...
% This function implements the detailed CCA methodology with adaptive sampling and validation
    ...
    try
        ...
        mean_A_matrix = cv_results.mean_A_matrix;
        mean_B_matrix = cv_results.mean_B_matrix;

        combined_data = [mean_A_matrix(:); mean_B_matrix(:)];

        % Now compute the true global minimum and maximum across both matrices
        min_global = min(combined_data);  % Single scalar value
        max_global = max(combined_data);  % Single scalar value
        ...
        cv_results.mean_A_matrix = (mean_A_matrix - min_global) / data_range;
        cv_results.mean_B_matrix = (mean_B_matrix - min_global) / data_range;
