...
def _draw_psth(
    ...
    if vmax is None:                                     # ← replace the
        vmax = max(float(np.nanpercentile(              #   existing single
...
def plot_summary_figure(
    ...
    ax = axes[1, 0]
    ax.plot(x, list( lat_i), "s-", color=_CI_PCCA, lw=1.8, ms=5, label=TARGET_I)
    ax.plot(x, list( lat_j), "^-", color=_CJ_PCCA, lw=1.8, ms=5, label=TARGET_J)
...
def plot_supplementary_panel(
    ...
    ax.set_title('(C)  Weight collinearity  κ\n'
                 r'κ → 1 : shared-collinearity; κ → 0 : clean separation',
                 fontsize=8.5)
...
def run_sequential_removal(
        ...
        session_name: str,
        n_show: int = N_NEURONS_SHOW,
        designated_region: Optional[str] = None,  # ← new
) -> None:

    out_part1 = output_dir / "part2_sequential"
    ...
    print(
        f"  [Part 2] Nuisance queue ({len(nuisance_queue)} regions): "
        f"{nuisance_queue}"
    )
    ...
    # ── Resolve designated (held-out) region ──────────────────────────────
    # The designated region is intentionally excluded from the sequential Z
    # accumulation so that its contribution can be compared in isolation
    # against the cumulative effect of all remaining nuisance regions.
    #
    # If no region is supplied by the caller, we perform a lightweight
    # single-region pCCA sweep (no figures) and select the region whose
    # removal diverges most from the plain CCA baseline (Z = ∅).  This
    # mirrors the divergence metric used in run_single_ablation and ensures
    # that the sequential analysis spotlights the dominant confounder.
    if designated_region is None:
        _Wx_cca_d, _Wy_cca_d, _rho_cca_d = ridge_cca(X_i_flat, X_j_flat)
        _cca_ref_d = StepResult(
            label="CCA_baseline",
            nuisance_regions=[],
            rho_pcca=float(_rho_cca_d[0]),
            Wx=_Wx_cca_d,
            Wy=_Wy_cca_d,
            z_i_mean=latent_projections(
                X_i_flat, _Wx_cca_d[:, 0], n_trials, T).mean(0),
            z_j_mean=latent_projections(
                X_j_flat, _Wy_cca_d[:, 0], n_trials, T).mean(0),
        )
        _abl_sweep: List[StepResult] = []
        for _r in nuisance_queue:
            _Wx_p, _Wy_p, _rho_p, _Xi_res, _Xj_res = pcca(
                X_i_flat, X_j_flat, nuisance_flat[_r])
            _abl_sweep.append(StepResult(
                label=f"sweep_{_r}",
                nuisance_regions=[_r],
                rho_pcca=float(_rho_p[0]),
                Wx=_Wx_p,
                Wy=_Wy_p,
                z_i_mean=latent_projections(
                    _Xi_res, _Wx_p[:, 0], n_trials, T).mean(0),
                z_j_mean=latent_projections(
                    _Xj_res, _Wy_p[:, 0], n_trials, T).mean(0),
            ))
        _, _idx_f_d, _sims_d = identify_extremes(
            _abl_sweep, _cca_ref_d, exclude_ref=False)
        designated_region = _abl_sweep[_idx_f_d].nuisance_regions[0]
        print(
            f"  [Part 2]  No designated_region supplied; "
            f"auto-selected '{designated_region}' "
            f"(D = {_sims_d[_idx_f_d]['divergence']:.3f} vs CCA baseline)."
        )
    elif designated_region not in nuisance_queue:
        raise ValueError(
            f"designated_region='{designated_region}' is absent from the "
            f"nuisance queue {nuisance_queue}.  Pass None for auto-selection "
            f"or supply one of the listed region names."
        )

    # Remove the designated region from sequential accumulation.
    # It remains in nuisance_flat / nuisance_z3d / global_sort so that
    # weight-bar neuron ordering stays consistent across all panels.
    nuisance_queue = [r for r in nuisance_queue if r != designated_region]
    print(
        f"  [Part 2]  Designated (held-out) region : '{designated_region}'\n"
        f"  [Part 2]  Sequential queue ({len(nuisance_queue)} steps): "
        f"{nuisance_queue}"
    )
    ...
    global_vmax = max(float(np.nanpercentile(np.abs(_all_psth), 99)), 0.5)
    ...
    for step_idx, add_region in enumerate(steps_iter):
        ...
        fig_path = out_part1 / (
            f"{session_name}_part2_step{step_idx:02d}"
    ...
    summary_fig = plot_summary_figure(
        ...
        output_path=out_part1 / f"{session_name}_part2_summary.png",
    )
    ...
    supp_fig = plot_supplementary_panel(
        ...
        output_path   = out_part1 / f'{session_name}_part2_supplementary.png',
    )
    plt.close(supp_fig)
...
def run_single_ablation(
    ...
    out_part2 = output_dir / "part1_ablation"
    ...
    print(f"  [Part 1] Full pCCA reference  ρ₁ = {rho_ref[0]:.4f}")
    ...
    for abl_idx, region in enumerate(nuisance_all):
        ...
        fig_path = out_part2 / f"{session_name}_part1_abl{abl_idx:02d}_{region}.png"
        fig.savefig(fig_path, dpi=200, bbox_inches="tight")
        step_fig_paths.append(fig_path)
        plt.close(fig)
    ...
    # ── Summary tables ───────────────────────────────────────────────────
    print("\n" + "=" * 90)
    print(f"Part 1  [{TARGET_I} ↔ {TARGET_J}]  "
    ...
    # compile_steps_to_video(
    #     fig_paths   = step_fig_paths,
    #     output_path = out_part2 / f"{session_name}_part2_ablation_video.mp4",
    #     fps         = 0.8,
    #     dpi         = 150,
    #     title       = (
    #         f"Sequential cumulative removal  |  {session_name}\n"
    ...
    fig_a = plot_summary_figure(
        ...
        output_path=out_part2 / f"{session_name}_part1_summary_vs_CCA.png",
    )
    plt.close(fig_a)
    ...
    supp_fig_abl = plot_supplementary_panel(
            ...
            output_path   = out_part2 / f'{session_name}_part1_supplementary.png',
        )
    plt.close(supp_fig_abl)

    most_divergent_region: str = ablation_results[idx_f_a].nuisance_regions[0]
    print(
        f"\n  [Part 1]  Most divergent from CCA baseline: "
        f"'{most_divergent_region}'  "
        f"(D = {sims_vs_cca[idx_f_a]['divergence']:.3f})  "
        f"→ passed to run_sequential_removal as designated_region."
    )
    return most_divergent_region
...
SESSIONS_TO_RUN = [
# Sessions recording MOs + VALVM + ORB :
#     'yp020_220331',
#     'yp020_220407',
#     'yp021_220404',
#     'yp021_220407'
...
def run_single_session(SESSION_NAME: str) -> None:
    ...
    target_pair_name = f"{TARGET_I}_{TARGET_J}"

    # 2. 将新层级加入到 OUTPUT_DIR 的构建中
    OUTPUT_DIR = BASE_DIR / "Paper_output" / "pcca_max_ablation_vs_other_region" / SESSION_NAME / target_pair_name
    ...
    # ── Run ablation FIRST so the most-divergent region is known ─────────
    print("\n── Part 1: Single-region ablation ─────────────────────────────────")
    designated_region: str = run_single_ablation(
    ...
    # ── Sequential removal, holding out the designated region ─────────────
    # print("\n── Part 2: Sequential cumulative removal ──────────────────────────")
    ...
    #     output_dir=OUTPUT_DIR,
    #     session_name=SESSION_NAME,
    #     designated_region='ORB',  # ← propagated from ablation
    # )
