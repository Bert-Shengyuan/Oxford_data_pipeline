...
def _draw_psth(
        ...
        region_name: str,
        n_show: int,
        vmax: Optional[float] = None,   # ← add this
) -> None:
...
def plot_step_panel(
        *,
        ...
        psth_vmax: Optional[float] = None,   # ← add this keyword argument
) -> plt.Figure:
    """
    2-row × 7-column diagnostic panel for one analysis step.
    ...
    6 : Nuisance → target regression weight  (β·w₁, barh, colour-coded;
          nu_w_i for row 0 / nu_w_j for row 1;
          abscissa shares the same sign convention as cols 1 and 3)

    Notes
    -----
    * Cols 0, 1, 3 use the per-region sort indices derived from the global
      Rastermap fit (see compute_global_neuron_order).
    * Col 5 and col 6 use the nuisance region's global-Rastermap sort index
      (sort_z), so their orderings are mutually consistent.
    * nu_w_i / nu_w_j may be None (e.g. CCA baseline step with Z = ∅), in
      which case col 6 displays a greyed-out placeholder.
    """
    fig, axes = plt.subplots(
        2, 7,
        figsize=(26, 7.5),
        gridspec_kw={
            "width_ratios": [3.5, 1.2, 2.5, 1.2, 2.5, 3.5, 1.2],
            "hspace": 0.52,
            "wspace": 0.32,
        },
    )
    ...
    row_cfg = [
        (
            ...
            Wx_pcca[:, 0], Wx_cca[:, 0], _CI_PCCA, _CI_CCA, nu_w_i,
        ),
        (
            TARGET_J, X_j_z3d, X_j_flat, X_j_res, sort_j,
            Wy_pcca[:, 0], Wy_cca[:, 0], _CJ_PCCA, _CJ_CCA, nu_w_j,
        ),
    ]

    for row, (
            rname, X_z3d, X_flat, X_res, sort_idx,
            w_pcca, w_cca, c_pcca, c_cca, nu_w,
        ...
        # ── Col 6 — nuisance regression weight β·w₁ ─────────────────────
        z_label = Z_name if Z_name is not None else "Z"
        if nu_w is not None and sort_z is not None:
            _draw_weight_bar(
                axes[row, 6], nu_w, sort_z, n_show,
                f"Z→{rname}\nβ·w₁",
            )
        else:
            axes[row, 6].text(
    ...
    if Z_z3d is not None and sort_z is not None and Z_name is not None:
        _draw_psth(axes[0, 5], Z_z3d, sort_z, time_vec, f"Nuisance: {Z_name}", n_show,vmax=psth_vmax)
        _draw_psth(axes[1, 5], Z_z3d, sort_z, time_vec, f"Nuisance: {Z_name}", n_show,vmax=psth_vmax)
    else:
        # ── Col 5 — nuisance PSTH (span both rows) ───────────────────────────
...
def main() -> None:
    ...
    SESSION_NAME = "yp021_220407"
    SESSION_FILE = (
            BASE_DIR
            / "pcca_sessions_cued_hit_long_results"
            / f"{SESSION_NAME}_analysis_results.mat"
    )
    OUTPUT_DIR = BASE_DIR / "Paper_output" / "pcca_ablation_7panel" / SESSION_NAME
